...
class PenggunaController extends BaseController
{
    public function index()
    {
        return view('halaman/pengguna/table', [
            'xx'    => (new PenggunaModel_12200180())->get()->getResult(),
    ...
    public function simpan(){
        ...
        if( $this->validasi() ){
            $r = (new PenggunaModel_12200180())->insert($data);
            if($r == false){
    ...
    public function edit($id){
        $data = (new PenggunaModel_12200180())->where('id', $id)->first();
    ...
    public function patch(){
        ...
        if( $this->validasiPatch() ){
            $r = (new PenggunaModel_12200180())->update($id, $data);
            if($r == true){
                return redirect()->to('/pengguna-list');
            }else{
    ...
    public function delete(){
        $id = $this->request->getPost('id');
        $r = (new PenggunaModel_12200180())->delete($id);
